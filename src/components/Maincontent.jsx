...
const MainContent = ({ ingredients, isUserLoggedIn }) => {
  ...
  const generateImage = async (recipeName, index) => {
    // Set loading state to true for the image at the specific index
    setLoadingImages((prev) => ({ ...prev, [index]: true }));

    try {
      // Send the recipeName to the backend to generate an image
      const response = await customFetch.post('/auth/createImage', {
        recipeName,
      });

      // Update the recipes state with the newly generated image for the corresponding index
      setRecipes((prev) => {
        const updatedRecipes = [...prev];
        updatedRecipes[index].image = response.data.generated_image; // assuming 'generated_image' is returned in the response
        return updatedRecipes;
      });
    } catch (error) {
      console.error('Error generating image:', error);
    } finally {
      // Set loading state to false after the request is finished
